...
typedef struct
{
    __IO uint32_t  WTR:1;	   //清零看门狗定时器
	__IO uint32_t  WTRE:1;	   //使能看门狗复位
	__IO uint32_t  WTRF:1;	   //看门狗定时器引发复位
 	__IO uint32_t  WTIF:1;	   //看门狗定时器中断标志 写1 清零
	__IO uint32_t  WTWKE:1;	   //使能看门狗定时器唤醒
	__I  uint32_t  WTWKF:1;	   //唤醒标志
  	__IO uint32_t  WTIE:1;	   //看门狗定时器中断使能
	__IO uint32_t  WTE:1;	   //看门狗定时器使能
 	__IO uint32_t  WTIS:3;	   //看门狗定时器溢出选择	5--1S
...
#define CPU_RST(x)        	*((__IO uint32_t *)(GCR_BASE + 0x08)) = 0x01;
