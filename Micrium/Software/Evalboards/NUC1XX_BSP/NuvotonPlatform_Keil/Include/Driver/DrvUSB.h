...
#define _DRVUSB_ENABLE_WAKEUP_INT()   USBD->INTEN.WAKEUP = 1
#define _DRVUSB_DISABLE_WAKEUP_INT()  USBD->INTEN.WAKEUP = 0
#define _DRVUSB_ENABLE_FLDET_INT()      USBD->INTEN.FLD = 1
#define _DRVUSB_DISABLE_FLDET_INT()     USBD->INTEN.FLD = 0
#define _DRVUSB_ENABLE_USB_INT()      USBD->INTEN.USB = 1
#define _DRVUSB_DISABLE_USB_INT()     USBD->INTEN.USB = 0
#define _DRVUSB_ENABLE_BUS_INT()      USBD->INTEN.BUS = 1
#define _DRVUSB_DISABLE_BUS_INT()     USBD->INTEN.BUS = 0
...
typedef struct 
{
	uint8_t Struct_Name;		//指令名称
	uint8_t Struct_Status;		//指令执行状态，00接收成功，01执行成功，02失败，03其他
	uint8_t Sys_Status;			//系统状态，01 U盘 02 PC-CAM 03机器显示状态（接了USB）04 机器显示状态，未接USB
	uint8_t Sen_Status;			//sensor状态，01 全幅 02 局部 03 细节局部 04 C608 
	uint8_t Scale_Status;		//显示比例状态，01 16:9  02 4:3 
	uint8_t Menu_Status;		//菜单状态，01 预览模式 02 回放 03其他
	uint8_t SD_Status;			//SD卡状态，01 插入 02 拔出 
	uint8_t LED_Status;			//光源状态
}Status;
