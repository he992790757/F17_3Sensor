...
uint32_t DrvPWM_SetTimerClk(uint8_t u8Timer, S_DRVPWM_TIME_DATA_T *sPt)
{
	...
	switch (u8Timer)
	{
		...
		case DRVPWM_CAP0:
			if (s_i32Option)
				outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & CCR_MASK) | BIT0);
			else
				outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & ~(BIT4 | BIT20)) | BIT0);	
			break;
		case DRVPWM_CAP1:
			if (s_i32Option)
				outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & CCR_MASK) | BIT16);
			else
				outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & ~(BIT4 | BIT20)) | BIT16);	
			break;
		case DRVPWM_CAP2:
			if (s_i32Option)
				outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & CCR_MASK) | BIT0);
			else
				outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & ~(BIT4 | BIT20)) | BIT0);	
			break;
		case DRVPWM_CAP3:
			if (s_i32Option)
				outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & CCR_MASK) | BIT16);
			else
				outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & ~(BIT4 | BIT20)) | BIT16);
			break;
		case DRVPWM_CAP4:
			outp32(&PWMB->CCR0, (inp32(&PWMB->CCR0) & ~(BIT4 | BIT20)) | BIT0);
			break;
		case DRVPWM_CAP5:
			outp32(&PWMB->CCR0, (inp32(&PWMB->CCR0) & ~(BIT4 | BIT20)) | BIT16);
			break;
		case DRVPWM_CAP6:
			outp32(&PWMB->CCR2, (inp32(&PWMB->CCR2) & ~(BIT4 | BIT20)) | BIT0);
			break;
		case DRVPWM_CAP7:
			outp32(&PWMB->CCR2, (inp32(&PWMB->CCR2) & ~(BIT4 | BIT20)) | BIT16);
			break;
	} 
...
/*           Note: The option is only applied to PWM0~3 capture function of some of NuMicro NUC100 Series. */
/*                 Please refer to data sheet in detail                                                    */	
/*---------------------------------------------------------------------------------------------------------*/
