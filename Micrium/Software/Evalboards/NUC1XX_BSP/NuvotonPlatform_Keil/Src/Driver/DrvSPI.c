...
/*         bQFN36PIN    [in]: Specify the package type                                                          */
/*                            TURE : QFN 36-pin package                                                         */
/*                            FALSE: other packages                                                             */
/*                                                                                                              */
/* Returns:                                                                                                     */
...
int32_t DrvSPI_Open(E_DRVSPI_PORT eSpiPort, E_DRVSPI_MODE eMode, E_DRVSPI_TRANS_TYPE eType, int32_t i32BitLength, uint8_t bQFN36PIN)
{
	...
	if(eSpiPort == eDRVSPI_PORT0)
	{
		...
		if(bQFN36PIN)                          /* for QFN 36-pin package */
			SYS->GPDMFP.SPI2_CLK_SPI0_SS1 =1;  /* for SS01 */
		else
...
/*    When enable 2-bit transfer mode, the Tx_NUM must be configure as 0x00 (one transaction in one transfer.)        */
/*--------------------------------------------------------------------------------------------------------------------*/
void DrvSPI_Set2BitTransferMode(E_DRVSPI_PORT eSpiPort, uint8_t bEnable)
{
	if(bEnable)
	{
		...
		if(eSpiPort == eDRVSPI_PORT0)
		{
			if(g_sSpiHandler[eSpiPort].bQFN36PinPackage)     /* for QFN 36-pin package */
			{
				SYS->GPDMFP.SPI2_MISO0_SPI0_MISO1 = 1;
				SYS->GPDMFP.SPI2_MOSI0_SPI0_MOSI1 = 1;
			}
...
/* Parameters:        	                                                                                    */
/*    eSpiPort     [in]: Specify the SPI port                                                               */
/*    eOption      [in]: the options of Byte Reorder function.                                              */
