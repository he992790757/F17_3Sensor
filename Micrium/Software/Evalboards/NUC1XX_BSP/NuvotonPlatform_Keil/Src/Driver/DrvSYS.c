...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*               u32Adj		- [in]	RC adjustment value. The value is 0x00~0x3F  					       */
/*                     		                           	   												   */
/* Returns:                                                                                                */
/*               None 										 											   */
/*              					                                                           			   */
...
int32_t DrvSYS_SetClockDivider(E_SYS_IP_DIV eIpDiv , int32_t i32value)
{
	...
	{
		SYSCLK->CLKDIV.CAN_N     =  i32value & 0x00F;
		SYSCLK->CLKDIV.CAN_N_EXT = (i32value & 0x3F0)>>4;
	}
...
{
    SYSCLK->PLLCON.OE = i32Flag;			 //0-使能 PLL FOUT
	SYSCLK->PLLCON.PD = i32Flag;			 //0-PLL正常模式
}
...
//	E_SYS_PLL_CLKSRC ePllSrc = E_SYS_INTERNAL_22M;
	DrvSYS_SelectPLLSource(ePllSrc);			   //选择外部12M晶振作为PLL时钟源
